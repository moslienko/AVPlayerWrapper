...
#elseif os(macOS)
        self.options.session.audioEngine.stop()
#endif
...
private extension AVPlayerWrapper {
    ...
    func setupObservers() {
        ...
        // Status observer
        playerItemStatusObserver = self.player?.currentItem?.observe(
            \.status,
             options: [.old, .new, .initial],
             changeHandler: { item, change in
                 onMainThread { [weak self] in
                     self?.didUpdateStatus?(item.status)
                     self?.delegate?.didUpdateStatus(status: item.status)
                 }
             }
        )
    }
